...
(function (dressup_game) {
    var AppScreen = (function () {
        function AppScreen(name, content) {
            ...
            this._reserv = Math.abs(this.content.getChildAt(0).x);
    ...
    var SceneScreen = (function (_super) {
        ...
        SceneScreen.prototype.initModelPart = function (part) {
            ...
            if (cfg.allowHide) {
                ...
                part.mousedown = function (it) { return part.visible = false; };
                part.touchstart = function (it) { return part.visible = false; };
            }
            part.visible = part.gotoLabel('default_frame');
        };
        SceneScreen.prototype.initModelButtons = function () {
            var _this = this;
            var clips = this.content.getAllByPrefix(SceneScreen.PART_BTN_PREFIX);
            for (var _i = 0; _i < clips.length; _i++) {
                var clip = clips[_i];
                var btn = new fl.Button(clip);
                btn.onPress = function (it) { return _this.onPartButtonClick(it); };
                this._partButtons.push(btn);
            }
        };
